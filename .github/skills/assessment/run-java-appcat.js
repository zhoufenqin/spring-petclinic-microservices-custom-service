...
function runAppcat(executable, args) {
  // Mirror the proven desktop-CLI invocation (see AssessmentRunner in the .NET CLI):
  //   * stdin  = 'ignore'  -> appcat gets an immediately-EOF /dev/null, never an open
  //                          pseudo-terminal it could block on.
  //   * stdout/stderr = 'pipe' (NOT inherited terminal fds) -> appcat detects a
  //                          non-TTY and stays in non-interactive mode. A GraalVM
  //                          native image attached to a REAL tty enters a JLine/
  //                          interactive code path that reads terminal capabilities
  //                          from stdin and then hangs forever — this is the
  //                          "report.json written, status Complete, but the process
  //                          never exits" hang seen on the coding agent terminal.
  // We stream the piped output line-by-line so the agent still sees liveness, and
  // because we stream (instead of spawnSync's in-memory buffer) there is no maxBuffer
  // limit that could kill appcat mid-run on a chatty project.
  return new Promise((resolve) => {
    ...
    try {
      ...
      log(`Failed to launch appcat (${exc && exc.message ? exc.message : exc}).`);
      resolve(1);
      return;
    }

    // Heartbeat: a JVM/GraalVM process block-buffers its stdout/stderr when the
    // destination is a pipe (not a TTY), so appcat's own log lines do not reach us
    // until its buffer fills or it exits — during a multi-minute analyze the agent
    // would otherwise see total silence and wrongly conclude the command is hung
    // (then kill + re-run it, producing duplicate reports). We emit our own
    // periodic liveness line so the agent always sees forward progress.
    const startedAt = Date.now();
    const heartbeat = setInterval(() => {
      const elapsed = Math.round((Date.now() - startedAt) / 1000);
      log(`appcat analyze still running... ${elapsed}s elapsed (this is normal; appcat can take several minutes on CI).`);
    }, 15000);
    ...
    const forwardLines = (stream) => {
      ...
      stream.on('data', (chunk) => {
        ...
        while ((nl = pending.indexOf('\n')) >= 0) {
          ...
          if (line.length) log(`appcat | ${line}`);
        }
      });
      stream.on('end', () => {
        const tail = pending.replace(/\r$/, '');
        if (tail.length) log(`appcat | ${tail}`);
      });
    };
    ...
    child.on('error', (exc) => {
      log(`appcat process error (${exc && exc.message ? exc.message : exc}).`);
      clearInterval(heartbeat);
      resolve(1);
    });
    // Resolve on 'exit' (the appcat process itself has terminated), NOT 'close'.
    // 'close' waits for every piped stdio stream to also close; if appcat is a
    // launcher that forks a worker which inherits our stdout/stderr pipe, that
    // worker can keep the pipe open after appcat exits, so 'close' would never
    // fire and this Promise would hang forever — exactly the "report.json written
    // but the runner never returns" symptom. 'exit' fires as soon as appcat is
    // gone; the report is already on disk and the .then() guard force-exits Node.
    child.on('exit', (code) => {
      clearInterval(heartbeat);
      const elapsed = Math.round((Date.now() - startedAt) / 1000);
      log(`appcat exited with code ${code === null ? 'null (signal)' : code} after ${elapsed}s.`);
...
async function main() {
  ...
  try {
    ...
    const exitCode = await runAppcat(executable, analyzeArgs);
