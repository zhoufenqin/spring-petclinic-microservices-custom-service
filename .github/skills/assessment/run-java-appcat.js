...
function downloadWithCurl(url, part) {
  ...
  // has built-in redirect following (-L), retries (--retry) and resume (-C -). Returns
  // true on success; on any failure we fall back to the Node https implementation.
  const args = [
    '-L', // follow redirects
  ...
  // stdio inherited so curl's own progress meter doubles as a liveness signal.
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
    for (const [sourceIndex, url] of ordered.entries()) {
      try {
        ...
        let done = false;
        if (curlAvailable()) {
          done = downloadWithCurl(url, part);
          if (!done) {
