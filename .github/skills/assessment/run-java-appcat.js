...
function runAppcat(executable, args) {
  ...
  // appcat streams its own per-rule progress to the console (inherited stdout/stderr),
  // so the analysis phase is self-evidently alive without extra heartbeat logging.
  // stdin is set to 'ignore' (an immediately-EOF /dev/null) rather than inherited:
  // appcat is a GraalVM native image whose terminal/JLine handling can block forever
  // trying to read an open-but-never-fed stdin (as happens in the coding agent's
  // non-interactive pseudo-terminal). With 'ignore', any stdin read returns EOF at
  // once, so appcat exits cleanly after writing report.json instead of hanging.
  const completed = spawnSync(executable, args, { stdio: ['ignore', 'inherit', 'inherit'] });
  if (completed.error) {
    return 1;
  }
