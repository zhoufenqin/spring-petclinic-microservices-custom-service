...
function runAppcat(executable, args) {
  ...
  //   * stdin  = 'ignore'  -> appcat gets an immediately-EOF stdin, so it can never
  ...
  // (no separate heartbeat needed). We stream (rather than buffer like spawnSync)
  // so there is no maxBuffer limit that could kill appcat mid-run on a chatty
  // project.
