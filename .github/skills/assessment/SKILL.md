...
2. **Run Assessment**:
   - **For Java projects**: First acquire AppCAT into a shared cache, then run the bundled script against it.
     - **Acquire AppCAT** (use a fast native downloader; skip the download if it is already cached):
       1. Detect the platform key as `{os}-{arch}` where `os` is `linux`, `macos`, or `windows`, and `arch` is `amd64` or `arm64` (e.g. `linux-amd64`, `windows-arm64`).
       2. Read `<skill-dir>/appcat-java-manifest.json` and take `version` plus `platforms[{platform-key}].url`. This URL is the official first-party release CDN link.
       3. If `~/.appcat/{version}/{platform-key}` already contains an `appcat` launcher, reuse it and skip the download.
       4. Otherwise download that URL into `~/.appcat` with `curl -L` (the archive is `.tar.gz` for linux/macos and `.zip` for windows). No SHA-256 verification is required — it is a first-party release we publish and control.
       5. Extract the archive into `~/.appcat/{version}/{platform-key}/` (`tar -xzf` for `.tar.gz`, `tar -xf` for `.zip`).
     - **Run** the bundled script, pointing it at the extracted AppCAT:
       - `node <skill-dir>/run-java-appcat.js --workspace-path {workspace-path} --appcat-home ~/.appcat/{version}/{platform-key}`
       - With `--appcat-home` the script downloads nothing; it only runs `appcat analyze` and writes the versioned report.
...
- **For Java**: Downloads AppCAT into `~/.appcat` (using the manifest URL for the detected platform), then runs the bundled `run-java-appcat.js` script with `--appcat-home` to run analysis and save the report to the versioned directory
