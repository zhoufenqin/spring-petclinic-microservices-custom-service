...
2. **Run Assessment**:
   - **For Java projects**: Acquiring AppCAT (Step 1) is a precondition for running the assessment (Step 2). Step 2 only works once an `appcat` launcher exists under `~/.appcat`, so always complete Step 1 first.
     ...
     - **Step 2 — Run the bundled script** (only after Step 1; it finds AppCAT under `~/.appcat` automatically):
       ...
       - Run it in the **foreground and wait for it to finish** — `appcat analyze` can take several minutes on a large project, and the script forwards appcat's own progress output line-by-line so you can see it is still working. Do **not** launch it as a background/async command and poll it, and do **not** kill and re-run it if it seems quiet: doing so produces duplicate `report.json` directories.
